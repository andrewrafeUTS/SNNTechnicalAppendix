...
inpt = InputNode(14)
out = IzhikevichNode(4)

#hidden = IzhikevichNode(n=50)

# Create the connections between the layers
in_out = AbstractConnection(source=inpt, target=out, wmin=0.0, wmax=50.0)
...
def update_weights(network, time, all_actions, all_rewards, all_source_spikes, all_target_spikes, all_total_rewards, all_ltp):
    ...
    worst_episodes = np.argpartition(np.asarray(all_total_rewards), 50)[:50]
    #print(all_ltp)
    #print(all_ltp)
    #print(len(all_ltp[0]))
...
def random_mutations():
    ...
    for i in range(4): # Output neurons
        for j in range(14): #input neurons
...
for e in range(num_eps):
    ...
    while not done and step < steps:
        #env.render()
        obs = preprocess(obs)
        enc = probabilities(obs)
        inpt.Process(enc)
        action, out_time = net.Run(inpt, time, False, decoding_method)

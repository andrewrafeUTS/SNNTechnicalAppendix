...
class LunarLanderEvolution():
    ...
    def __init__(self, NeuronsPerLayer, ExposurePeriod: int = 50, Decoding: DecodingMethod = DecodingMethod.F2F_RESET):
        ...
        print("Trial Details: Exposure Period = " + str(self.ExposurePeriod) + " Decoding Method = " + str(self.Decoding))
    ...
    def Preprocess(self, obs):
        ...
        newObs = np.asarray([
            ...
            obs[6], # Left leg touched
            obs[7]  # Right leg touched
    ...
    def RunEpisode(self, AgentIndex: int, NumEpisodes: int, DisplayEpisode: bool, BestAgentEpisode: bool):
        ...
        for _ in range(NumEpisodes):
            ...
            for _ in range(1000):
                ...
                action = self.Population[AgentIndex].Run(newObs, self.ExposurePeriod, False, self.Decoding)
                #print(action)
        ...
        if BestAgentEpisode:
            ...
            if (float(RewardTotal)/float(NumEpisodes) >= 200.0):
        ...
        if DisplayEpisode:
            ...
            '''plt.bar(x=np.arange(0, 14), height=(self.Population[AgentIndex].Connections[0].w))
    ...
    def Evolve(self):
        ...
        #Percentage = int(len(self.Population)*0.1)
        ...
        if (float(BestAgentReward)/float(self.NumEpisodes) >= 200.0):
